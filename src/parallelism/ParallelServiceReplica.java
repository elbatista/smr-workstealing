...
public class ParallelServiceReplica extends ServiceReplica {
    ...
    public ParallelServiceReplica(int id, Executable executor, Recoverable recoverer, ClassToThreads[] cts){
        super(id, executor, recoverer);
    }
    
    public ParallelServiceReplica(int id, Executable executor, Recoverable recoverer, int initialWorkers, int duration, int warmup) {
        ...
        //statistics = new ThroughputStatistics(id, initialWorkers, "results_early_" + id + ".txt", "EARLY", duration, warmup);
        this.scheduler = new DefaultScheduler(initialWorkers, cts);
        initWorkers(this.scheduler.getNumWorkers(), id, duration, warmup);
    }

    public ParallelServiceReplica(int id, Executable executor, Recoverable recoverer, Scheduler s, int duration, int warmup) {
        //this(id, executor, recoverer, new DefaultScheduler(initialWorkers));
        super(id, executor, recoverer);
        this.scheduler = s;
        initWorkers(this.scheduler.getNumWorkers(), id, duration, warmup);
    ...
    public ParallelServiceReplica(int id, Executable executor, Recoverable recoverer, int initialWorkers, ClassToThreads[] cts, int duration, int warmup, ThroughputStatistics statistics) {
        //this(id, executor, recoverer, new DefaultScheduler(initialWorkers));
        super(id, executor, recoverer);
        if (initialWorkers <= 0) {
            initialWorkers = 1;
 ...
 /*public ParallelServiceReplica(int id, Executable executor, Recoverable recoverer, Scheduler s) {
    ...
    protected void initWorkers(int n, int id, int duration, int warmup) {
        ...
        statistics = new ThroughputStatistics(id, n, "results_early_" + id + ".txt", "EARLY", duration, warmup);
    ...
    protected class ServiceReplicaWorker extends Thread {

        protected FIFOQueue<MessageContextPair> requests;
