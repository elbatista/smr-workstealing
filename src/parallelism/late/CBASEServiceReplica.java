...
public class CBASEServiceReplica extends ParallelServiceReplica {
    ...
    public CBASEServiceReplica(int id, Executable executor, Recoverable recoverer, int numWorkers, ConflictDefinition cf, String graphType,
        ...
        super(id, executor, recoverer, new CBASEScheduler(cf, numWorkers, graphType), duration,  warmup);
    ...
    protected void initWorkers(int n, int id, int duration, int warmup) {
        statistics = new ThroughputStatistics(id, n, "results_cbase_" + id + ".txt", "CBASE", duration, warmup);
        int tid = 0;
        for (int i = 0; i < n; i++) {
    ...
    private class CBASEServiceReplicaWorker extends Thread {
        ...
        public CBASEServiceReplicaWorker(CBASEScheduler s, int id) {
            ...
            System.out.println("Starting a cbase thread: " + id);
        }
